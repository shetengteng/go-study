...
// 第一个参数是tableName
// 第二个参数是map，key是列名，value是值，表示待更新的键值对
var _update generator = func(values ...interface{}) (string, []interface{}) {
	// UPDATE $tableName SET x1 = ?,x2 = ?
	tableName := values[0]
	...
	return fmt.Sprintf("UPDATE %s SET %s", tableName, strings.Join(keys, ",")), params
}

// values 就一个值tableName
var _delete generator = func(values ...interface{}) (string, []interface{}) {
	return fmt.Sprintf("DELETE FROM %s", values[0]), []interface{}{}
}

// values的第一个值是tableName
var _count generator = func(values ...interface{}) (string, []interface{}) {
	return _select(values[0], []string{"count(1)"})
}
